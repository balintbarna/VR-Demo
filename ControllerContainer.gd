...
func display_transform(controller):
    ...
    ($LeftContainer/TransformContainer/Position as Label).text = "Position\nX:  {}\nY:  {}\nZ: {}".format(posarr, "{}")
    ($LeftContainer/TransformContainer/Rotation as Label).text = "Rotation\nX:  {}\nY:  {}\nZ: {}".format(rotarr, "{}")
...
func display_all_button_states(controller: VrController):
    ...
    ($RightContainer/AllButtons as Label).text = text
...
func display_all_axes_states(controller: VrController):
    ...
    ($LeftContainer/AllAxes as Label).text = text


func display_basics(controller: VrController):
    ($LeftContainer/Name as Label).text = "Name: {}".format([controller.get_name()], "{}")
    ($LeftContainer/Hand as Label).text = "Hand: {}".format([controller.get_hand()], "{}")
    ($LeftContainer/Active as Label).text = "Active: {}".format([controller.get_is_active()], "{}")
    ($LeftContainer/Joystick as Label).text = "Joystick: {}".format([controller.get_joystick_id()], "{}")
    ($LeftContainer/MeshUpdate as Label).text = "Mesh updated {} times".format([mesh_counter], "{}")
...
func _on_button_action(button: int, action: String) -> void:
    ($LeftContainer/ButtonEvent as Label).text = "Button {} {}".format(["%2d" % button, action], "{}")
